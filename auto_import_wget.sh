...
download_with_progress() {
    ...
    while [ $retry -lt $max_retries ]; do
        ...
        # Verificar se arquivo existe em diferentes formatos
        URL_GZ="$url"
        URL_SQL="${url%.gz}"  # Remove .gz se existir

        # Determinar URL correta
        if wget --spider -q "$URL_GZ" 2>/dev/null; then
            DOWNLOAD_URL="$URL_GZ"
            IS_GZIPPED=true
        elif wget --spider -q "$URL_SQL" 2>/dev/null; then
            DOWNLOAD_URL="$URL_SQL"
            IS_GZIPPED=false
        else
            echo -e "${RED}✗ Arquivo não encontrado em nenhum formato${NC}"
            return 1
        fi

        # Download com wget mostrando progresso (timeout de 5 minutos)
        if [ "$IS_GZIPPED" = true ]; then
            if wget --timeout=300 --tries=1 --progress=bar:force:noscroll "$DOWNLOAD_URL" -O "$SQL_DIR/$file.gz" 2>&1 | \
            grep --line-buffered "%" | \
            sed -u -e "s/.* \([0-9]\+\)%.*/\1/" | \
